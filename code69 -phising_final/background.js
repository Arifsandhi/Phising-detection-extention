async function checkUrlWithGoogleSafeBrowsing(url) {
  const apiKey = "AIzaSyDRc-q9PWJbFn6aSYrzHsra85Qbbjne2SM"; 
...
// function isClickjackingLink(link) {
//   // Placeholder function for clickjacking detection
//   return false;
// }
    ...
    const validateLinks = async () => {
      const results = await Promise.all(
        message.links.map(async (link) => ({
          ...
          // isClickjacking: isClickjackingLink(link),
        }))
      );
